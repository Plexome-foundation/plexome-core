...
void Node::init() {
    std::cout << "[System] Node: " << config_.node_id << " Initializing..." << std::endl;
    ...
    for (const auto& entry : std::filesystem::directory_iterator("./models")) {
        if (entry.path().extension() == ".gguf") {
            if (engine_->load_model(entry.path().string())) {
                std::cout << "[AI] Loaded: " << entry.path().filename() << std::endl;
                break; 
            }
        }
    }
...
void Node::run_cli() {
    ...
    while (is_running_) {
        ...
        if (input == "exit" || input == "quit") {
            ...
            std::cout << "[AI Core] Model ready." << std::endl;
        }
        else if (input.rfind("ask ", 0) == 0) {
            std::cout << "\n[AI]: " << engine_->predict(input.substr(4)) << "\n" << std::endl;
        }
        else {
